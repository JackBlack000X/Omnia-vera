...
struct TodayWidgetRowView: View {
  ...
  var body: some View {
      ...
      VStack(alignment: .leading, spacing: compact ? 4 : (ultraCompact ? 4 : 5)) {
        ...
        HStack(spacing: compact ? 4 : (ultraCompact ? 5 : 6)) {
          Text("\(item.currentCount)/\(item.targetCount)")
            .font(.system(size: compact ? 10 : (ultraCompact ? 10 : 11), weight: .bold))
            .monospacedDigit()
            .foregroundStyle(item.isComplete ? Color(red: 0.45, green: 0.98, blue: 0.68) : Color.white.opacity(0.8))
            .padding(.horizontal, compact ? 6 : (ultraCompact ? 6 : 7))
            .padding(.vertical, compact ? 2 : (ultraCompact ? 2 : 3))
        ...
        if #available(iOSApplicationExtension 17.0, *) {
          Button(intent: ToggleHabitIntent(habitId: item.action.habitId, logicalDate: item.action.logicalDate)) {
            Image(systemName: actionSymbol)
              .font(.system(size: compact ? 12 : (ultraCompact ? 13 : 14), weight: .bold))
              .foregroundStyle(.white)
          ...
          Link(destination: url) {
            Image(systemName: "arrow.up.right")
              .font(.system(size: compact ? 12 : (ultraCompact ? 12 : 13), weight: .bold))
              .foregroundStyle(.white)
              .frame(width: actionSize, height: actionSize)
              .background(Color.white.opacity(0.16))
              .clipShape(Circle())
    ...
    .background(Color.white.opacity(compact ? 0.07 : (ultraCompact ? 0.075 : 0.08)))
    .overlay(
...
struct TodayWidgetEntryView: View {
  ...
  private var visibleItems: ArraySlice<TodayWidgetItem> {
    switch family {
    case .systemSmall:
      return entry.snapshot.items.prefix(1)
    case .systemMedium:
  ...
  private var widgetContent: some View {
    VStack(alignment: .leading, spacing: isCompact ? 8 : (isMedium ? 10 : 12)) {
      ...
      if visibleItems.isEmpty {
        ...
        ForEach(Array(visibleItems)) { item in
          TodayWidgetRowView(item: item, compact: isCompact, ultraCompact: false)
        }
